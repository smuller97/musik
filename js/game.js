...
function preload () {
  // Loader billeder og spritesheet
  game.load.image('sky', 'asserts/sky.png');
  game.load.image('ground', 'asserts/platform.png');
  game.load.image('node', 'asserts/node.gif');
...
function create () {
  ...
  game.add.sprite(0, 0, 'sky');  //  Baggrund for spil
  // #region Platforme
  ...
  ledge = platforms.create(110, 300, 'ground');
  ledge.body.immovable = true;
  // #endregion
  ...
  for (var i = 0; i < 12; i++) {
    let node = nodes.create(i * 70, 0, 'node');
    node.body.gravity.y = 1000; //de kommer ned fra himlen
...
function update () {
 ...
 // #endregion
  ...
  if (score === 120) { //Tjekker om alle noder indsamlet
...
{ 
  window.location.replace("concert.html");
}
